...
const data =
    fs.readFileSync("C:/Users/jonat/source/repos/aoc2021/day11/input.txt", 'utf8')
    .split("\n")
    .slice(0,-1)
    .map((row) => {
        return (
...
const octoGeneration = (grid:DumboOctopus[][]) => {
    grid.map((row) => {
        row.map ((o) => {
...
const octoLoop = (grid:DumboOctopus[][], generations:number) => {
    ...
    for (let i = 0; i < generations; i++) {
        let nextGen = octoGeneration(currentGen)
        currentGen = nextGen
    }
    return currentGen
}
console.log(countFlashes(octoLoop(data,100)))